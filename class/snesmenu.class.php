...
/**
 * \file        class/snesmenu.class.php
 * \ingroup     mealplanner
 * \brief       This file is a CRUD class file for SnesMenu (Create/Read/Update/Delete)
 */
 ...
 * Class for SnesMenu
 */
class SnesMenu extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'snesmenu';

	/**
	 * @var string 	Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'mealplanner_snesmenu';

	/**
	 * @var string 	If permission must be checkec with hasRight('mealplanner', 'read') and not hasright('mymodyle', 'snesmenu', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'mealplanner';

	/**
	 * @var string 	String with name of icon for snesmenu. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'snesmenu@mealplanner' if picto is file 'img/object_snesmenu.png'.
	 */
	...
	//  */
	// public $table_element_line = 'mealplanner_snesmenuline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_snesmenu';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'SnesMenuline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'SnesMenu', 'fk_element'=>'fk_snesmenu'));
	...
	//  */
	// protected $childtablesoncascade = array('mealplanner_snesmenudet');

	// /**
	//  * @var SnesMenuLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('mealplanner', 'snesmenu', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('mealplanner', 'snesmenu', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('mealplanner', 'snesmenu_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'snesmenu/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'snesmenu/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'snesmenu/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'snesmenu/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->mealplanner->dir_output.'/snesmenu/'.$oldref;
				$dirdest = $conf->mealplanner->dir_output.'/snesmenu/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->mealplanner->dir_output.'/snesmenu/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowSnesMenu")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("SnesMenu").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/mealplanner/snesmenu_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowSnesMenu");
	...
	public function getLinesArray()
	{
		...
		$objectline = new SnesMenuLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_snesmenu:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('MEALPLANNER_MYOBJECT_ADDON')) {
			$conf->global->MEALPLANNER_MYOBJECT_ADDON = 'mod_snesmenu_standard';
		}
		...
		if (getDolGlobalString('MEALPLANNER_MYOBJECT_ADDON')) {
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefSnesMenu $obj';
...
/**
 * Class SnesMenuLine. You can also remove this and generate a CRUD class for lines objects.
 */
class SnesMenuLine extends CommonObjectLine
{
	// To complete with content of an object SnesMenuLine
	// We should have a field rowid, fk_snesmenu and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'snesmenu'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_snesmenu'

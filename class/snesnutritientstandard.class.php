...
/**
 * \file        class/snesnutritientstandard.class.php
 * \ingroup     mealplanner
 * \brief       This file is a CRUD class file for SnesNutritientStandard (Create/Read/Update/Delete)
 */
...
/**
 * Class for SnesNutritientStandard
 */
class SnesNutritientStandard extends CommonObject
{
	/**
	 * @var string 	ID of module.
	 */
	public $module = 'mealplanner';

	/**
	 * @var string 	ID to identify managed object.
	 */
	public $element = 'snesnutritientstandard';

	/**
	 * @var string 	Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'mealplanner_snesnutritientstandard';

	/**
	 * @var string 	If permission must be checkec with hasRight('mealplanner', 'read') and not hasright('mymodyle', 'snesnutritientstandard', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'mealplanner';

	/**
	 * @var string 	String with name of icon for snesnutritientstandard. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'snesnutritientstandard@mealplanner' if picto is file 'img/object_snesnutritientstandard.png'.
	 */
	...
	//  */
	// public $table_element_line = 'mealplanner_snesnutritientstandardline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_snesnutritientstandard';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'SnesNutritientStandardline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'SnesNutritientStandard', 'fk_element'=>'fk_snesnutritientstandard'));
	...
	//  */
	// protected $childtablesoncascade = array('mealplanner_snesnutritientstandarddet');

	// /**
	//  * @var SnesNutritientStandardLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('mealplanner', 'snesnutritientstandard', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('mealplanner', 'snesnutritientstandard', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('mealplanner', 'snesnutritientstandard_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'snesnutritientstandard/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'snesnutritientstandard/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'snesnutritientstandard/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'snesnutritientstandard/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->mealplanner->dir_output.'/snesnutritientstandard/'.$oldref;
				$dirdest = $conf->mealplanner->dir_output.'/snesnutritientstandard/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->mealplanner->dir_output.'/snesnutritientstandard/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('mealplanner','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('mealplanner','mealplanner_advance','validate'))))
		 {
		 $this->error='Permission denied';
		 return -1;
		 }*/

		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'MEALPLANNER_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'MEALPLANNER_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'MEALPLANNER_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowSnesNutritientStandard")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("SnesNutritientStandard").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/mealplanner/snesnutritientstandard_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowSnesNutritientStandard");
	...
	public function getLinesArray()
	{
		...
		$objectline = new SnesNutritientStandardLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_snesnutritientstandard:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('MEALPLANNER_MYOBJECT_ADDON')) {
			$conf->global->MEALPLANNER_MYOBJECT_ADDON = 'mod_snesnutritientstandard_standard';
		}

		if (getDolGlobalString('MEALPLANNER_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('MEALPLANNER_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('MEALPLANNER_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/mealplanner/");
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefSnesNutritientStandard $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_snesnutritientstandard';
		...
		$modelpath = "core/modules/mealplanner/doc/";
...
/**
 * Class SnesNutritientStandardLine. You can also remove this and generate a CRUD class for lines objects.
 */
class SnesNutritientStandardLine extends CommonObjectLine
{
	// To complete with content of an object SnesNutritientStandardLine
	// We should have a field rowid, fk_snesnutritientstandard and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'snesnutritientstandard'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_snesnutritientstandard'

...
/**
 *   	\file       snesnutritientstandard_list.php
 *		\ingroup    mealplanner
 *		\brief      List page for snesnutritientstandard
 */
...
require_once __DIR__.'/class/snesnutritientstandard.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('mealplanner', 'snesnutritientstandard', 'write');
	$permissiontodelete = $user->hasRight('mealplanner', 'snesnutritientstandard', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("mealplanner")) {
	accessforbidden('Module mealplanner not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'SnesNutritientStandard';
	$objectlabel = 'SnesNutritientStandard';
	$uploaddir = $conf->mealplanner->dir_output;
...
$title = $langs->trans("SnesNutritientStandards");
//$help_url = "EN:Module_SnesNutritientStandard|FR:Module_SnesNutritientStandard_FR|ES:Módulo_SnesNutritientStandard";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/mealplanner/snesnutritientstandard_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-mealplanner page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/mealplanner/snesnutritientstandard_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendSnesNutritientStandardRef";
$modelmail = "snesnutritientstandard";
$objecttmp = new SnesNutritientStandard($db);

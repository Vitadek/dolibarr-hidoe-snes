...
/**
 *   	\file       snesmenu_list.php
 *		\ingroup    mealplanner
 *		\brief      List page for snesmenu
 */
...
require_once __DIR__.'/class/snesmenu.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('mealplanner', 'snesmenu', 'write');
	$permissiontodelete = $user->hasRight('mealplanner', 'snesmenu', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'SnesMenu';
	$objectlabel = 'SnesMenu';
...
$title = $langs->trans("SnesMenus");
//$help_url = "EN:Module_SnesMenu|FR:Module_SnesMenu_FR|ES:Módulo_SnesMenu";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/mealplanner/snesmenu_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/mealplanner/snesmenu_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendSnesMenuRef";
$modelmail = "snesmenu";
$objecttmp = new SnesMenu($db);
